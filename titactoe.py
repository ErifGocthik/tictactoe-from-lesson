...
class TicTacToe:
    ...
    def move(self):  # ход
        def is_move_valid(x, y): # Проверка наличия координат места куда ходит игрок
            return 1 <= x <= 3 and 1 <= y <= 3 and self.table[x-1][y-1] == '*' # Возвращение True если x или y находятся в диапозоне хода и False в ином случае

        print(f'Ход игрока {self.get_current_player_name()}:')
        is_coords_valid = False
        x, y = 0, 0
        while not is_coords_valid: # запускаем цикл проверки хода
            y, x = input('Введите координаты x и y через пробел: ').split() # Вводим координаты
            x, y = int(x), int(y) #Преобразуем написаные нами координаты в число
            is_coords_valid = is_move_valid(x, y) # Привсваеваем результат проверки
    ...
    def get_winner(self):  # поиск победителя
        count_stars = 0
        for line in self.table: # Запускаем цикл
            if line[0] != '*' and len(set(line)) == 1:
                self.winner = self.current_move # Победителю присваевается текущий ход
                return None
            if '*' in line:
                count_stars += 1

        for i in range(3):
            if '*' != self.table[0][i] == self.table[1][i] == self.table[2][i]: # Проверка ряда по диагонали
    ...
    def reload(self):  # перезагрузка игры
        print('Началась игра!')
        self.table = [['*', '*', '*'], ['*', '*', '*'], ['*', '*', '*']] # Создаем двухмерную матрицу
        if self.winner == '' or self.winner == 'n':
            self.current_move = 'x' # Присваеваем ход х
        else:
    ...
    def start(self):  # начало игры
        while True:
            ...
            if self.winner:  # если победитель есть - завершаем игру
                break
            self.change_current_move() #Меняем знак
